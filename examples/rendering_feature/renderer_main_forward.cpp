...
constexpr const char* kShadingPipeline = "phong_shadow_mat";
...
int main() {
    ...
    vkkk::ForwardRenderer renderer(&ctx, &scene);
    if (!renderer.initialize(&ctx)) {
        throw std::runtime_error("failed to initialize ForwardRenderer");
    }

    if (!renderer.create_pipeline_from_shader_src(
            kShadingPipeline,
            vkkk::phong_shadow_vert,
            vkkk::phong_shadow_frag,
            make_pipeline_option(),
            {vkkk::VERTEX, vkkk::NORMAL}))
    {
        throw std::runtime_error("failed to create phong_shadow_mat pipeline");
    }

    if (!ctx.bind_pipeline_depth_attachment(
            kShadingPipeline, 1, renderer.shadow_depth_attachment_index()))
    {
        throw std::runtime_error("failed to bind shadow map to phong_shadow_mat");
    }
    ...
    ctx.set_resize_cbk([&](uint32_t w, uint32_t h) {
        ...
        // Fixed-size shadow map survives recreate_swapchain; rebind in case descriptors moved.
        ctx.bind_pipeline_depth_attachment(
            kShadingPipeline, 1, renderer.shadow_depth_attachment_index());
    });
    ...
    renderer.add_opaque_drawable("cornell_plane", kShadingPipeline, phong_attr_size,
    ...
    renderer.add_opaque_drawable("cornell_cube", kShadingPipeline, phong_attr_size,
    ...
    const auto update = [&](float duration) {
        ...
        ImGui::Text("Renderer: Forward (Shadow)");
    ...
    const auto record = [&](vk::raii::CommandBuffer& cmd_buf, uint32_t idx) {
