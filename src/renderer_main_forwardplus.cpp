    ...
    uint32_t image_index, const vkkk::built_in_shader::PhongLightUBO& light_ubo)
{
    ...
    auto& transform_ubo = ctx.require_ubo(batch.pipeline_name + ":UniformBufferObject");
    auto& material_ubo = ctx.require_ubo(batch.pipeline_name + ":PhongMaterial");
    auto& light_ubo_mem = ctx.require_ubo(batch.pipeline_name + ":PhongLight");
    auto& instance_attrs = ctx.require_ubo(batch.pipeline_name + ":InstanceAttrs");
...
int main() {
    vkkk::Context ctx;
    GLFWwindow* window = ctx.init_glfw(WIDTH, HEIGHT, "Forward+ Instanced Cornell");
    ...
    ctx.set_update_cbk([&](uint32_t idx, float duration) {
        ...
        light_ubo.lightColor = glm::vec4(1.0f, 1.0f, 1.0f, 1.0f);
